...
class ViewController: UIViewController, UIScrollViewDelegate{
    ...
    @IBOutlet weak var PropertyMenuView: UIView!
        @IBOutlet var nameInfoBox: UITextField!
        @IBOutlet var xPosInfoBox: UITextField!
        @IBOutlet var yPosInfoBox: UITextField!
        @IBOutlet var widthInfoBox: UITextField!
        @IBOutlet var heightInfoBox: UITextField!
        @IBOutlet var angleInfoBox: UITextField!
        @IBOutlet var colourSelectOne: UISegmentedControl!
        @IBOutlet var colourSelectTwo: UISegmentedControl!
        @IBOutlet var colourSelectThree: UISegmentedControl!
        @IBOutlet var colourSelectFour: UISegmentedControl!
    ...
    var curtains1 = UIImageView()
    var curtainsView1 = UIView()
    var curtains2 = UIImageView()
    var curtainsView2 = UIView()
    ...
    @objc override func viewDidLoad() {
        ...
        //curtain creation
        curtains1 = UIImageView.init(image: UIImage(named: "Set Shapes"))
        curtains1.frame = CGRect(x: 0, y: 0, width: 225, height: 15)
        curtainsView1.frame = CGRect(x: 17*BackgroundImage.frame.width/30, y: BackgroundImage.frame.height/6.5, width: 0, height: 0)
        curtainsView1.transform = CGAffineTransform(rotationAngle: 1.25)
        curtainsView1.addSubview(curtains1)
        BackgroundImage.addSubview(curtainsView1)
        
        curtains2 = UIImageView.init(image: UIImage(named: "Set Shapes"))
        curtains2.frame = CGRect(x: 0, y: 0, width: 190, height: 15)
        curtainsView2.frame = CGRect(x: 3*BackgroundImage.frame.width/4, y: BackgroundImage.frame.height/1.7, width: 0, height: 0)
        curtainsView2.transform = CGAffineTransform(rotationAngle: 3.78)
        curtainsView2.addSubview(curtains2)
        BackgroundImage.addSubview(curtainsView2)
...
//curser_Button------------------------------------------------------------------------------------------------------------------curser_Button
    ...
    @IBAction func addActor(_ sender: Any) {
        ...
        items.append(Item(x: Int(( halfScreenWidth-Float(BackgroundImage.positionIn(view: MyView).origin.x))*(1/screenAspect)),
                          ...
                          name: "Unnamed Actor",
                          type: "Actor Shape")!)
        BackgroundImage.addSubview(items[items.count-1].rotateView)
    ...
    func drawArrow(ani: Int){
        ...
        var preY = Float(0.0)
        print(ani)
        for i in 0...animations.count-1{//find origin
            if  i == animations.count-1{
                preX = Float(animations[ani].identity.x)
                preY = Float(animations[ani].identity.y)
                break
            }
            if animations[ani].identity == animations[(ani-i)-1].identity{
                preX = Float(animations[(ani-i)-1].x)
                preY = Float(animations[(ani-i)-1].y)
                break
            }
            if i == animations.count-1{
                preX = Float(animations[ani].identity.x)
    ...
    func UpdateAnimationMenu(){
        ...
        if aniScrollView.subviews.count > 2 { //remove old animations
            for i in 0...animations.count-1{
                animations[i].view.removeFromSuperview()
            }
        }
        ...
        if animations.count > 0 {
            for i in 0...animations.count-1{
                if animations[i].simple == true{ //draw closed
                    space = animations[i].drawSimple(num: space)
                }else{
                    if animations[i].open == true{ //draw open
    ...
    func AnimateList(loop: Int,end: Int){
        ...
        if loop < end-1{ // count the number of simutanius animations
        ...
        for j in 0...numTogether{
            if animations[loop+j].time > time { //save longest animation time
                time = animations[loop+j].time
            }
            
            animations[loop+j].view.backgroundColor = UIColor.green//highlight
            
            UIView.animate(withDuration: animations[loop+j].time) {
                if self.animations[loop+j].simple == true{//animate curtains
                    self.curtains1.frame = CGRect(x: self.curtains1.frame.origin.x, y: self.curtains1.frame.origin.y,
                                           width: CGFloat(self.animations[loop+j].percent*(225/100)), height: self.curtains1.frame.height)
                    self.curtains2.frame = CGRect(x: self.curtains1.frame.origin.x, y: self.curtains1.frame.origin.y,
                                           width: CGFloat(self.animations[loop+j].percent*(190/100)), height: self.curtains1.frame.height)
                }else{//animatre item
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + time) {//wait for animations to finish
            for j in 0...numTogether{// unhighlight
            ...
            numTogether+=1
            if loop+numTogether<end{// if any animations left
                if self.animating{// if still runing
                    self.AnimateList(loop: loop+numTogether, end: end)
                }else{
                    self.aniStep = loop+numTogether//pause
                }
            }else{//stop
                self.aniStep = 0
