...
class Animator: NSObject {
    ...
    init?(x: Int, y: Int, time: Double, identity: Item, type: Bool, screen: Int){
        self.x = x
        self.y = y
        self.time = time
    ...
    func drawClose(num: Int) -> Int{
        ...
        yBox.text = String(Int(screen-self.y)-(self.identity.height+20))
    ...
    func drawOpen(num: Int) -> Int{
        ...
        let upButt     = UIButton.init(   frame: CGRect.init(x: 4,           y: (4*height/5), width: (width/3)-8,   height: (height/5)-4))
        let downButt   = UIButton.init(   frame: CGRect.init(x: (width/3),   y: (4*height/5), width: (width/3)-4,   height: (height/5)-4))
        let delete     = UIButton.init(   frame: CGRect.init(x: (2*width/3), y: (4*height/5), width: (width/3)-4,   height: (height/5)-4))
        ...
        rePos.setTitle("REpos", for: .normal)
        ...
        downButt.setTitle("Down", for: .normal)
   ...
   //     sender.backgroundColor = UIColor.red
    ...
    @IBAction func editY(_ sender: UITextField) {
        self.y = Int(screen)-((Int(sender.text!) ?? 0)+(self.identity.height+20))
    }
