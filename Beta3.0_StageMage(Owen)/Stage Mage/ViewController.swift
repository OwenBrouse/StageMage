...
//        MenuButton.frame.height = MenuView.frame.height
        ...
        c1.frame = CGRect(x: 0, y: 0, width: 190, height: 15)
        ch1.frame = CGRect(x: 63*BackgroundImage.frame.width/120, y: BackgroundImage.frame.height/7, width: 0, height: 0)
        ...
        ch2.frame = CGRect(x: 21*BackgroundImage.frame.width/30, y: BackgroundImage.frame.height/2, width: 0, height: 0)
        ch2.transform = CGAffineTransform(rotationAngle: 3.75)
        ch2.addSubview(c2)
...
//ReactionFuntion#############################################################################################################################ReactionFuntion
            ...
            for i in 0...items.count-1{
                if items[i].select == true {
                    animations.append(Animator(x: Int(MyView.frame.width/2), y: Int(MyView.frame.height/2),
                                               time: 1, identity: items[i],type: false, screen: Int(MyView.frame.height),simple: false)!)
    ...
    @IBAction func addCurtainAni(_ sender: Any) {
        animations.append(Animator(x: 0, y: 0, time: 1, identity: items[0], type: false, screen: 0, simple: true)!)
        animations[animations.count-1].open = true
        UpdateAnimationMenu()
    }
